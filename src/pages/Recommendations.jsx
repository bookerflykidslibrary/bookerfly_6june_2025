...
export default function Catalog({ user }) {
  ...
  const fetchReadBooks = async (email) => {
    ...
    const limit = (plan?.NumberOfBooks || 0) + 2;

    const [readRes, futureRes] = await Promise.all([
    ...
    const readISBNs = (readRes?.data || []).map(b => b.ISBN13?.trim().toLowerCase());
    const futureISBNs = (futureRes?.data || []).map(b => b.ISBN13?.trim().toLowerCase());

    if (futureISBNs.length >= limit) {
      alert(`You have already selected ${futureISBNs.length} books. You can only select up to ${limit}.`);
    }

    setHiddenRead([...new Set([...readISBNs, ...futureISBNs])]);
  };
  ...
  return (
    <div className="p-4 max-w-7xl mx-auto bg-gradient-to-br from-blue-50 to-pink-50 min-h-screen">
      ...
      {hiddenRead?.length > 0 && <p className="text-sm text-red-600 font-medium mb-4">Hidden books already read or requested by you.</p>}
          ...
          {books.map(book => (
            <div key={book.BookID} className="bg-white rounded-xl p-4 shadow-md flex flex-col">
              ...
              <p className="text-xs text-gray-800 mt-1">
                {book.Description?.length > 120 ? (expandedDesc[book.BookID] ? book.Description : `${book.Description?.substring(0, 120)}... `) : book.Description}
                {book.Description?.length > 120 && <span onClick={() => toggleDescription(book.BookID)} className="text-blue-500 cursor-pointer underline">more</span>}
              </p>
              ...
              <div className="mt-2 text-xs text-gray-700">
                <div className="flex flex-col sm:flex-row gap-2">
                  ...
                  <a href={`https://www.amazon.in/s?k=${encodeURIComponent(book.Title + ' ' + book.ISBN13)}&tag=vandana1230b9-21`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-orange-600 hover:underline">Buy on Amazon <FaExternalLinkAlt /></a>
                </div>
              </div>
            </div>
          ))}
