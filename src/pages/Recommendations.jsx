...
export default function AdminSignUpRequests() {
  ...
  const updateStatus = async (id, newStatus, request) => {
    ...
    if (error) {
      ...
      if (newStatus === 'APPROVED') {
        const message = `Hi ${request.name}, thanks for signing up with Bookerfly Kids Library! We're excited to have you onboard 📚😊`;
        await sendWhatsAppMessage(request.phone, message);
      }
      fetchRequests();
    }
  };
              ...
              <tr key={r.id}>
                ...
                <td className="border p-2 space-x-2">
                  <button
                    ...
                    onClick={() => updateStatus(r.id, 'APPROVED', r)}
                  >
                    Approve
                  </button>
                  <button
                    className="bg-red-500 text-white px-2 py-1 rounded"
                    onClick={() => updateStatus(r.id, 'REJECTED', r)}
                  >
                    Reject
                  </button>
                </td>
